
import 'package:mortalheart_mall/common/util/color_util.dart';

class CommonStyle {
  static ColorUtil splashColor = ColorUtil.hex2Color("#ffc93d");
  static ColorUtil themeColor = ColorUtil.hex2Color("#FF1520");
  static ColorUtil menuBjlor = ColorUtil.hex2Color("#F2F2F2");
  static ColorUtil searchColor = ColorUtil.hex2Color("#808080");
  static ColorUtil selectedTabColor = ColorUtil.hex2Color("#CC3250");
  static ColorUtil gradientleftColor = ColorUtil.hex2Color("#85B1F9");
  static ColorUtil gradientRightColor = ColorUtil.hex2Color("#79BBF8");
  static ColorUtil gradientCenterColor = ColorUtil.hex2Color("#85B5F9");
  static ColorUtil gradientbottomColor = ColorUtil.hex2Color("#89BFF7");
  static ColorUtil primaryColor = ColorUtil.hex2Color("#3E3F40");
  static ColorUtil unSelectedTabColor = ColorUtil.hex2Color("#433D33");
  static ColorUtil greyBgColor = ColorUtil.hex2Color("#F5F5F4");
  static ColorUtil selectedMeuColor = ColorUtil.hex2Color("#FF1520");
  static ColorUtil greyBgColor2 = ColorUtil.hex2Color("#F1F1F2");



  static ColorUtil colorF3F3F3 = ColorUtil.hex2Color("#F3F3F3");
  static ColorUtil colorF7CD6B = ColorUtil.hex2Color("#F7CD6B");
  static ColorUtil colorF8D47F = ColorUtil.hex2Color("#F8D47F");
  static ColorUtil colorF1F1F1 = ColorUtil.hex2Color("#F1F1F1");
  static ColorUtil color333333 = ColorUtil.hex2Color("#333333");
  static ColorUtil color969798 = ColorUtil.hex2Color("#969798");
  static ColorUtil color777677 = ColorUtil.hex2Color("#777677");
  static ColorUtil colorE6E6E6 = ColorUtil.hex2Color("#E6E6E6");

}
